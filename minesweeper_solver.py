...
class MinesweeperSolver:
    ...
    def __init__(self, settings=mg.GAME_DOTA_LVL_1, helper=None):
        ''' Initiate the solver.
        IN:
...
def main(game_level):
    ...
    '''
    if game_level == 1:
        settings = mg.GAME_DOTA_LVL_1
    elif game_level == 2:
        settings = mg.GAME_DOTA_LVL_2
    elif game_level == 3:
        settings = mg.GAME_DOTA_LVL_3
    elif game_level == 4:
        settings = mg.GAME_DOTA_LVL_4
    elif game_level == 5:
        settings = mg.GAME_DOTA_LVL_5
    else:
        print("Invalid game level. Choose 1 -> 5.")
        return
    print(f"Game level: {game_level}")
    game = mg.MinesweeperGame(settings, seed=0)

    # For testing: show the game field
    print('Print field2str game')
    ...
    if once:
        ...
        # mid_x, mid_y = settings.shape[0] // 2, settings.shape[1] // 2
        mid_x = random.randint(0, settings.shape[0] - 1)
        mid_y = random.randint(0, settings.shape[1] - 1)
        # Append the middle coordinate to the safe list
        safe.append((mid_x, mid_y))
        game.make_a_move(safe, mines)
        once = False

    while game.status == mg.STATUS_ALIVE:
        safe, mines = solver.solve(game.uncovered) # Call the function
        ...
        if chance is not None and float(chance) < 0.6 and (mines is None or len(mines) == 0):
            pyautogui.press('1')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Choose the game level to run.')
    parser.add_argument('game_level', type=int, choices=[1, 2, 3, 4, 5], help='')

    # Parse the command-line arguments
    args = parser.parse_args()

    # Call the main function with the chosen game level
    try:
        while True:
            main(args.game_level)
